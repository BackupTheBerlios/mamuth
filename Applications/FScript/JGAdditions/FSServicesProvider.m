...
- (void)registerExports;
{
  ...
  [self registerServerConnection:@"fs"]; // tell Philipe to remove this one!
  ...
  for (i=0;(i<count) && !doExit;i++) {
    ...
    if (![command isEqualToString:@""]) {
      ...
      if (!result) {
        doExit=YES;
        if (multiLineMode)
          result=[NSString stringWithFormat:@"FScript error with command at line %d: %@",i+1,command];
        else
